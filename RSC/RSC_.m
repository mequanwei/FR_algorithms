...
%PROC_NUM          =   700;
NewTrain_DAT      =   tra(:,1:end-1)';
D_labels          =   tra(:,end)';
NewTest_DAT       =   tes(:,1:end-1)';
testlabels        =   tes(:,end)';
...
residual          =   [];
eigen_num         =   100;  % the dimensionality of eigenface
lambda            =   0.001;
MEDC              =   [0.8];
BETC              =   [8];

[disc_set,disc_value,Mean_Image]=Eigenface_f(NewTrain_DAT,100);
...
acc = sum(ID == testlabels)/length(testlabels);
