function final_result=TPTSR_test(A)
% two stage test sample sparse representation test
M=10:10:40;
test_M=size(M,2);
test_num=10; %experiment times
...
sample_num=10;
mu=0.01;       
train_num=9;         %samples used for trainning in each class
test_sample_num=1;   %samples used in tests in each class
[n_A, m_A]=size(A);
...
%     train_sample_index= 1:5;%[1,3,5,7,9,11,13,15];%1:5;
%     train_num=size(train_sample_index,2);%samples used for trainning in each class
%     test_sample_index=5:10;%[2,4,6,8,10,12,14];%6:10;
